...
type Buffer[F Frame[T], T Type] struct {
	buf      []F    // contents are the samples buf[off : len(buf)]
...
func (b *Buffer[F, T]) tryGrowByReslice(n int) (int, bool) {
...
func (b *Buffer[F, T]) grow(n int) int {
	m := b.Len()
	...
	if b.buf == nil && n <= smallBufferSize {
		b.buf = make([]F, n, smallBufferSize)
		return 0
	}
	c := cap(b.buf)
	if n <= c/2-m {
		...
		b.buf = growSlice[F, T](b.buf[b.off:], b.off+n)
	}
...
func (b *Buffer[F, T]) Write(p []F) (n int, err error) {
	b.lastRead = opInvalid
...
func (b *Buffer[F, T]) ReadFrom(r Reader[F, T]) (n int64, err error) {
	b.lastRead = opInvalid
	for {
		i := b.grow(MinRead)
		b.buf = b.buf[:i]
...
func growSlice[F Frame[T], T Type](b []F, n int) []F {
	defer func() {
		if recover() != nil {
			fmt.Printf("%s\n", ErrTooLarge.Error())
		}
...
func (b *Buffer[F, T]) WriteTo(w Writer[F, T]) (n int64, err error) {
	b.lastRead = opInvalid
...
// WriteFrame appends the frame to the buffer, growing the buffer as needed.
// The returned error is always nil, but is included to match [bufio.Writer]'s
// WriteFrame. If the buffer becomes too large, WriteSample will panic with
// [ErrTooLarge].
func (b *Buffer[F, T]) WriteFrame(frame F) error {
	b.lastRead = opInvalid
...
// Read reads the next len(p) frames from the buffer or until the buffer
// is drained. The return value n is the number of samples read. If the
// buffer has no data to return, err is [io.EOF] (unless len(p) is zero);
// otherwise it is nil.
func (b *Buffer[F, T]) Read(p []F) (n int, err error) {
	b.lastRead = opInvalid
	if b.empty() {
...
func (b *Buffer[F, T]) Next(n int) []F {
	b.lastRead = opInvalid
	m := b.Len()
	if n > m {
		n = m
...
func (b *Buffer[F, T]) ReadSample() (F, error) {
	if b.empty() {
		// Buffer is empty, reset to recover space.
		b.Reset()
		return *new(F), io.EOF
...
func (b *Buffer[F, T]) UnreadSample() error {
...
func NewBuffer[F Frame[T], T Type](buf []F) *Buffer[F, T] { return &Buffer[F, T]{buf: buf} }
